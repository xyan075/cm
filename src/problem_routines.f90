...
999 CALL ERRORS("PROBLEM_SOLVER_EQUATIONS_STATIC_NONLINEAR_SOLVE",ERR,ERROR)
  ...
  SUBROUTINE PROBLEM_SOLVER_GEOMETRIC_TRANSFORMATION_SOLVE(SOLVER,ERR,ERROR,*)
   ...
   !Argument variables
    ...
    INTEGER(INTG) :: numberOfComponents,componentIdx,nodeIdx,solverIdx
    INTEGER(INTG) :: VERSION_NUMBER,DERIVATIVE_NUMBER,MESH_COMPONENT_NUMBER,NODE_LOCAL_NUMBER,ITERATION_NUMBER
      ...
      IF(SOLVER%SOLVE_TYPE==SOLVER_GEOMETRIC_TRANSFORMATION_TYPE) THEN
        IF(ASSOCIATED(SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER)) THEN
          IF(ASSOCIATED(SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER%FIELD)) THEN
            ...
            FIELD_VARIABLE=>FIELD%VARIABLE_TYPE_MAP(SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER%VARIABLE_TYPE)%PTR
            numberOfComponents=SIZE(SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER%TRANSLATION,1)
            VERSION_NUMBER=1;
            DERIVATIVE_NUMBER=1;
            ITERATION_NUMBER=SOLVER%SOLVERS%CONTROL_LOOP%LOAD_INCREMENT_LOOP%ITERATION_NUMBER 
            !Find the boundary condition variable that store the final location of the fixed nodes 
            DO solverIdx=1,SOLVER%SOLVERS%NUMBER_OF_SOLVERS
              SOLVER_EQUATIONS=>SOLVER%SOLVERS%SOLVERS(solverIdx)%PTR%SOLVER_EQUATIONS
              IF(ASSOCIATED(SOLVER_EQUATIONS)) THEN
            ...
            DO componentIdx=1,numberOfComponents
              ...
              IF(ASSOCIATED(DOMAIN)) THEN
                ...
                DO nodeIdx=1,DOMAIN_NODES%NUMBER_OF_NODES
                  NODE_LOCAL_NUMBER=DOMAIN_NODES%NODES(nodeIdx)%LOCAL_NUMBER
                  !Translate nodes
                  CALL FIELD_PARAMETER_SET_ADD_LOCAL_NODE(FIELD,SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER%VARIABLE_TYPE, &
                    & FIELD_VALUES_SET_TYPE,VERSION_NUMBER,DERIVATIVE_NUMBER,NODE_LOCAL_NUMBER,componentIdx, &
                    & SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER%TRANSLATION(componentIdx,ITERATION_NUMBER),ERR,ERROR,*999)
                  IF(ASSOCIATED(BOUNDARY_CONDITIONS_VARIABLE)) THEN
                    IF(BOUNDARY_CONDITIONS_VARIABLE%DOF_COUNTS(BOUNDARY_CONDITION_FIXED_INCREMENTED)>0) THEN
                      !Translate corresponding boundary condition 
                      CALL FIELD_PARAMETER_SET_ADD_LOCAL_NODE(FIELD,SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER%VARIABLE_TYPE, &
                        & FIELD_BOUNDARY_CONDITIONS_SET_TYPE,VERSION_NUMBER,DERIVATIVE_NUMBER,NODE_LOCAL_NUMBER,componentIdx, &
                        & SOLVER%GEOMETRIC_TRANSFORMATION_SOLVER%TRANSLATION(componentIdx,ITERATION_NUMBER),ERR,ERROR,*999)
                    ENDIF
                  ENDIF
                ENDDO !nodeIdx
