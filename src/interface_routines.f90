...
999 CALL ERRORS("INTERFACE_MESH_CONNECTIVITY_INITIALISE",ERR,ERROR)
  ...
  SUBROUTINE INTERFACE_POINTS_CONNECTIVITY_FINALISE(INTERFACE_POINTS_CONNECTIVITY,ERR,ERROR,*)
    ...
    INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES=0
...
999 CALL ERRORS("INTERFACE_POINTS_CONNECTIVITY_CREATE_START",ERR,ERROR)
  ...
  !>Finalises the meshes connectivity and deallocates all memory
  SUBROUTINE INTERFACE_POINTS_CONNECTIVITY_ELEMENT_NUMBER_SET(POINTS_CONNECTIVITY,DATA_POINT_USER_NUMBER,COUPLED_MESH_IDX, &
      & ELEMENT_USER_NUMBER,ERR,ERROR,*)
      
    !Argument variables
    TYPE(INTERFACE_POINTS_CONNECTIVITY_TYPE), POINTER :: POINTS_CONNECTIVITY !<A pointer to interface points connectivity to set the element number of elements for.
    INTEGER(INTG), INTENT(IN) :: DATA_POINT_USER_NUMBER !<The index of the data point.
    INTEGER(INTG), INTENT(IN) :: COUPLED_MESH_IDX !<The index of the coupled mesh in the interface to set the number of elements for.
    INTEGER(INTG), INTENT(IN) :: ELEMENT_USER_NUMBER !<The coupled mesh element user number
    INTEGER(INTG), INTENT(OUT) :: ERR !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: ERROR !<The error string
    !Local Variables
    INTEGER(INTG) :: NumberOfXiDirections !<Number of XI directions of the coupled mesh
    ...
    IF(ASSOCIATED(POINTS_CONNECTIVITY)) THEN
      IF(POINTS_CONNECTIVITY%POINTS_CONNECTIVITY_FINISHED) THEN
        CALL FLAG_ERROR("Interface points connectivity has already been finished.",ERR,ERROR,*999)
      ELSE
        CALL DATA_POINT_CHECK_EXISTS(POINTS_CONNECTIVITY%INTERFACE%DATA_POINTS,DATA_POINT_USER_NUMBER,DATA_POINT_EXISTS, &
          & DATA_POINT_GLOBAL_NUMBER,ERR,ERROR,*999)
        IF(DATA_POINT_EXISTS) THEN
          IF ((COUPLED_MESH_IDX>POINTS_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES).OR.(COUPLED_MESH_IDX<0)) THEN
            CALL FLAG_ERROR("Interface coupled mesh index number out of range.",ERR,ERROR,*999)
          ELSE
            IF (ALLOCATED(POINTS_CONNECTIVITY%POINTS_CONNECTIVITY)) THEN
              CALL MESH_TOPOLOGY_ELEMENT_CHECK_EXISTS(POINTS_CONNECTIVITY%INTERFACE%COUPLED_MESHES(COUPLED_MESH_IDX)%PTR, &
                & MESH_COMPONENT_NUMBER,ELEMENT_USER_NUMBER,ELEMENT_EXISTS,ELEMENT_GLOBAL_NUMBER,ERR,ERROR,*999)       
              IF(ELEMENT_EXISTS) THEN
                POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(DATA_POINT_GLOBAL_NUMBER,COUPLED_MESH_IDX)%COUPLED_MESH_ELEMENT_NUMBER= &
                  & ELEMENT_GLOBAL_NUMBER
                NumberOfXiDirections=POINTS_CONNECTIVITY%INTERFACE%COUPLED_MESHES(COUPLED_MESH_IDX)%PTR%NUMBER_OF_DIMENSIONS
                IF(.NOT.ALLOCATED(POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(DATA_POINT_GLOBAL_NUMBER,COUPLED_MESH_IDX)%XI)) THEN
                  ALLOCATE(POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(DATA_POINT_GLOBAL_NUMBER,COUPLED_MESH_IDX)% &
                    & XI(NumberOfXiDirections,1))
                ENDIF
              ELSE
...
999 CALL ERRORS("INTERFACE_POINTS_CONNECTIVITY_ELEMENT_NUMBER_SET",ERR,ERROR)
  ...
  !>Set the interface points connectivity xi location
  SUBROUTINE INTERFACE_POINTS_CONNECTIVITY_POINT_XI_SET(POINTS_CONNECTIVITY,DATA_POINT_USER_NUMBER,COUPLED_MESH_IDX, &
    & ELEMENT_USER_NUMBER,MESH_COMPONENT_NUMBER,XI,ERR,ERROR,*)
          ...
          IF(DATA_POINT_EXISTS) THEN
            ...
            ELSE
              CALL MESH_TOPOLOGY_ELEMENT_CHECK_EXISTS(POINTS_CONNECTIVITY%INTERFACE%COUPLED_MESHES(COUPLED_MESH_IDX)%PTR, &
                & MESH_COMPONENT_NUMBER,ELEMENT_USER_NUMBER,ELEMENT_EXISTS,ELEMENT_GLOBAL_NUMBER,ERR,ERROR,*999)       
              IF(ELEMENT_EXISTS) THEN
                IF(MESH_COMPONENT_NUMBER>0 .AND. MESH_COMPONENT_NUMBER<=POINTS_CONNECTIVITY%INTERFACE_MESH% & 
                    & NUMBER_OF_COMPONENTS) THEN
                  IF(POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(DATA_POINT_GLOBAL_NUMBER,COUPLED_MESH_IDX)% &
                    & COUPLED_MESH_ELEMENT_NUMBER/=ELEMENT_GLOBAL_NUMBER) THEN
                    CALL FLAG_ERROR("Coupled mesh element number doesn't match that set to the interface.",ERR,ERROR,*999)
                  ELSE
                    POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(DATA_POINT_GLOBAL_NUMBER,COUPLED_MESH_IDX)% &
                      & XI(:,MESH_COMPONENT_NUMBER)=XI(:) 
                  ENDIF
                ELSE
     ...
     IF(ASSOCIATED(INTERFACE_POINTS_CONNECTIVITY)) THEN
       ...
       ELSE
         ...
         ALLOCATE(INTERFACE_POINTS_CONNECTIVITY%COUPLED_MESH_ELEMENTS(INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_ELEMENTS, &
           & INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES),STAT=ERR)
         ALLOCATE(INTERFACE_POINTS_CONNECTIVITY%DATA_POINT_PROJECTED(INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_DATA_POINTS), &
           & STAT=ERR)
         INTERFACE_POINTS_CONNECTIVITY%DATA_POINT_PROJECTED=.TRUE.
         ALLOCATE(INTERFACE_POINTS_CONNECTIVITY%IS_PENETRATION(INTERFACE_POINTS_CONNECTIVITY%INTERFACE%COORDINATE_SYSTEM% &
           & NUMBER_OF_DIMENSIONS,INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_DATA_POINTS), &
           & STAT=ERR)
         INTERFACE_POINTS_CONNECTIVITY%IS_PENETRATION=.TRUE.
         ALLOCATE(INTERFACE_POINTS_CONNECTIVITY%NORMAL(INTERFACE_POINTS_CONNECTIVITY%INTERFACE%COORDINATE_SYSTEM% &
           & NUMBER_OF_DIMENSIONS,INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_DATA_POINTS),STAT=ERR)
         INTERFACE_POINTS_CONNECTIVITY%NORMAL=0.0_DP
         ALLOCATE(INTERFACE_POINTS_CONNECTIVITY%GAP(INTERFACE_POINTS_CONNECTIVITY%INTERFACE%COORDINATE_SYSTEM% &
           & NUMBER_OF_DIMENSIONS,INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_DATA_POINTS),STAT=ERR)
         INTERFACE_POINTS_CONNECTIVITY%GAP=0.0_DP
       ENDIF
     ELSE
       CALL FLAG_ERROR("Interface points connectivity is not associated.",ERR,ERROR,*999)
     ENDIF
...
999 CALL ERRORS("INTERFACE_POINTS_CONNECTIVITY_CREATE_FINISH",ERR,ERROR)
  ...
  SUBROUTINE INTERFACE_POINTS_CONNECTIVITY_CONTACT_NUMBER_CALCULATE(POINTS_CON,ERR,ERROR,*)
    ...
    DO COUPLED_MESHID=1,POINTS_CON%NUMBER_OF_COUPLED_MESHES
    ...
    IF(ASSOCIATED(POINTS_CON)) THEN
      ...
      ELSE
        IF (ALLOCATED(POINTS_CON%POINTS_CONNECTIVITY)) THEN
          ...
          ELSE
            IF((COUPLED_MESHID > POINTS_CON%NUMBER_OF_COUPLED_MESHES).OR.(COUPLED_MESHID < 0)) THEN
              CALL FLAG_ERROR("Interface coupled mesh index number out of range.",ERR,ERROR,*999)
            ELSE
        ...
        IF(ASSOCIATED(DATA_PROJECTION)) THEN
          ...
          IF(DATA_PROJECTION%DATA_PROJECTION_PROJECTED) THEN
            DO data_point_idx=1,POINTS_CON%NUMBER_OF_DATA_POINTS
              ...
              POINTS_CON%POINTS_CONNECTIVITY(data_point_idx,COUPLED_MESH_IDX)%COUPLED_MESH_ELEMENT_NUMBER= &
                & DATA_PROJECTION_RESULT%ELEMENT_NUMBER !The global element number
              IF(DATA_PROJECTION_RESULT%ELEMENT_LINE_NUMBER/=0) THEN
                CALL INTERFACE_POINTS_CONNECTIVITY_POINT_XI_CONTACT_SET(POINTS_CON,data_point_idx,COUPLED_MESH_IDX, &
                  ...
                  & DATA_PROJECTION_RESULT%XI,ERR,ERROR,*999) ! this subroutine takes global data point number and element numbers
    ...
    IF(ASSOCIATED(INTERFACE)) THEN
      IF(ASSOCIATED(INTERFACE%POINTS_CONNECTIVITY)) THEN
        ...
        ELSE
          ...
          INTERFACE%POINTS_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES=INTERFACE%NUMBER_OF_COUPLED_MESHES
          ALLOCATE(INTERFACE%POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(INTERFACE%POINTS_CONNECTIVITY%NUMBER_OF_DATA_POINTS, &
            & INTERFACE%POINTS_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES))
          DO I = 1, INTERFACE%POINTS_CONNECTIVITY%NUMBER_OF_DATA_POINTS
            DO J = 1, INTERFACE%POINTS_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES
              INTERFACE%POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(I,J)%COUPLED_MESH_ELEMENT_NUMBER=0
              INTERFACE%POINTS_CONNECTIVITY%POINTS_CONNECTIVITY(I,J)%COUPLED_MESH_CONTACT_NUMBER=0
            ENDDO! J
          ENDDO! I
        END IF
      ELSE
...
999 CALL ERRORS("INTERFACE_MESH_CONNECTIVITY_ELEMENT_FINALISE",ERR,ERROR)
  ...
  SUBROUTINE INTERFACE_POINTS_CONNECTIVITY_POINTS_FINALISE(INTERFACE_POINTS_CONNECTIVITY,ERR,ERROR,*) 
    ...
    DO I = 1,INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_DATA_POINTS  !<Loop over all the interface data points
      DO J = 1,INTERFACE_POINTS_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES  !<Loop over all coupled domains 
