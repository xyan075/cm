    ...
    IF(ASSOCIATED(INTERFACE_MESH_CONNECTIVITY)) THEN
      ...
      ELSE
        IF((INTERFACE_MESH_ELEMENT_NUMBER>0).AND.(INTERFACE_MESH_ELEMENT_NUMBER<= &
          ...
          IF((COUPLED_MESH_INDEX>0).AND.(COUPLED_MESH_INDEX<=INTERFACE_MESH_CONNECTIVITY%NUMBER_OF_COUPLED_MESHES)) THEN
            IF (ALLOCATED(INTERFACE_MESH_CONNECTIVITY%ELEMENT_CONNECTIVITY)) THEN
              IF (INTERFACE_MESH_CONNECTIVITY%INTERFACE%SELF_CONTACT .AND. &
                 & (INTERFACE_MESH_CONNECTIVITY%ELEMENT_CONNECTIVITY(INTERFACE_MESH_ELEMENT_NUMBER,COUPLED_MESH_INDEX)% &
                 & COUPLED_MESH_ELEMENT_NUMBER .NE. 0)) THEN !If the first region element number has been entered
                 MESH_IDX=COUPLED_MESH_INDEX+1
                 !MESH_IDX=COUPLED_MESHID
               ELSE
                 MESH_IDX=COUPLED_MESH_INDEX
          ...
          ELSE
             IF (ALLOCATED(INTERFACE_MESH_CONNECTIVITY%ELEMENT_CONNECTIVITY)) THEN
               ...
               ELSE
                 MESH_IDX=COUPLED_MESH_INDEX
               ENDIF
               INTERFACE_MESH_CONNECTIVITY%ELEMENT_CONNECTIVITY(INTERFACE_MESH_ELEMENT_NUMBER,MESH_IDX)% &
                 & COUPLED_MESH_ELEMENT_NUMBER=COUPLED_MESH_ELEMENT_NUMBER
               XI_DIR=INTERFACE_MESH_CONNECTIVITY%INTERFACE_MESH%NUMBER_OF_DIMENSIONS+1
               LOCAL_NODE=INTERFACE_MESH_CONNECTIVITY%BASIS%NUMBER_OF_NODES
               IF(.NOT.ALLOCATED(INTERFACE_MESH_CONNECTIVITY%ELEMENT_CONNECTIVITY(INTERFACE_MESH_ELEMENT_NUMBER,MESH_IDX)%XI)) THEN
                 ...
                 ALLOCATE(INTERFACE_MESH_CONNECTIVITY%ELEMENT_CONNECTIVITY(INTERFACE_MESH_ELEMENT_NUMBER,MESH_IDX)%XI(XI_DIR,1, &
                   & LOCAL_NODE))
               ENDIF
               INTERFACE_MESH_CONNECTIVITY%ELEMENT_CONNECTIVITY(INTERFACE_MESH_ELEMENT_NUMBER,MESH_IDX)%XI=0.0_DP
             ELSE
               CALL FLAG_ERROR("Interface elements connectivity array not allocated.",ERR,ERROR,*999)
             END IF
          END IF
...
999 CALL ERRORS("INTERFACE_MESH_CONNECTIVITY_ELEMENT_NUMBER_SET",ERR,ERROR)
  ...
  SUBROUTINE INTERFACE_MESH_CONNECTIVITY_CONNECTED_LINES_CALCULATE(INTERFACE_MESH_CONNECTIVITY,ERR,ERROR,*)
    ...
    DO CoupledMeshIdx=1,INTERFACE_MESH_CONNECTIVITY%INTERFACE%NUMBER_OF_COUPLED_MESHES
