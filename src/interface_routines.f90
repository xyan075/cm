              ...
              IF((COUPLED_ELEM>0).OR.(COUPLED_ELEM<MESH_CON%INTERFACE%COUPLED_MESHES(COUPLED_MESHID)%PTR%NUMBER_OF_ELEMENTS))THEN
                IF((COMP_NO>0).OR.(COMP_NO<MESH_CON%INTERFACE_MESH%NUMBER_OF_COMPONENTS+1)) THEN
                  IF((LOCAL_NODE>0).OR.(LOCAL_NODE<MESH_CON%BASIS%NUMBER_OF_NODES))THEN
                    ...
                    IF(MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_ELEMENT_NUMBER/=COUPLED_ELEM)THEN
                    ...
                    ELSE
                      MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(:,COMP_NO,LOCAL_NODE)=XI(:)
                    END IF
                  ELSE
...
999 CALL ERRORS("INTERFACE_MESH_CONNECTIVITY_ELEMENT_XI_SET",ERR,ERROR)
  ...
  SUBROUTINE INTERFACE_MESH_CONNECTIVITY_ELEMENT_XI_CONTACT_SET(MESH_CON,INT_ELEM,COUPLED_MESHID,COUPLED_ELEM,LOCAL_CONTACT, &
    ...
    ELSE
      MESH_IDX=COUPLED_MESHID
      MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_NUMBER=LOCAL_CONTACT
      IF (SIZE(XI,1)==MESH_CON%BASIS%NUMBER_OF_XI) THEN
        SELECT CASE(MESH_CON%BASIS%NUMBER_OF_XI)
        CASE(1)
          SELECT CASE(LOCAL_CONTACT)
          CASE(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=0.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=XI(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=-2
          CASE(2)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=1.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=XI(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=2
          CASE(3)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=0.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=XI(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=-1
          CASE(4)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=1.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=XI(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=1
          CASE DEFAULT 
        ...
        CASE(2)
          ...
          CASE(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=1.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=XI(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(3,COMP_NO,LOCAL_NODE)=XI(2)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=1
          CASE(2)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=0.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=XI(1)
          ...
          CASE(4)
            ...
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=-2
          CASE(5)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(3,COMP_NO,LOCAL_NODE)=1.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=XI(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=XI(2)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=3
          CASE(6)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(3,COMP_NO,LOCAL_NODE)=0.0_DP
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(1,COMP_NO,LOCAL_NODE)=XI(1)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%XI(2,COMP_NO,LOCAL_NODE)=XI(2)
            MESH_CON%ELEMENTS_CONNECTIVITY(INT_ELEM,MESH_IDX)%COUPLED_MESH_CONTACT_XI_NORMAL=-3
          CASE DEFAULT 
  ...
  SUBROUTINE INTERFACE_MESH_CONNECTIVITY_ELEMENT_FINALISE(INTERFACE_MESH_CONNECTIVITY,ERR,ERROR,*) 
    ...
    DO I = 1,INTERFACE_MESH_CONNECTIVITY%NUMBER_INT_ELEM  !<Loop over all the interface mesh elements
      DO J = 1,INTERFACE_MESH_CONNECTIVITY%NUMBER_INT_DOM  !<Loop over all coupled domains 
        IF (ALLOCATED(INTERFACE_MESH_CONNECTIVITY%ELEMENTS_CONNECTIVITY)) THEN
          INTERFACE_MESH_CONNECTIVITY%ELEMENTS_CONNECTIVITY(I,J)%COUPLED_MESH_ELEMENT_NUMBER=0
          INTERFACE_MESH_CONNECTIVITY%ELEMENTS_CONNECTIVITY(I,J)%COUPLED_MESH_CONTACT_NUMBER=0
          INTERFACE_MESH_CONNECTIVITY%ELEMENTS_CONNECTIVITY(I,J)%COUPLED_MESH_CONTACT_XI_NORMAL=0
          IF(ALLOCATED(INTERFACE_MESH_CONNECTIVITY%ELEMENTS_CONNECTIVITY(I,J)%XI)) THEN
            DEALLOCATE(INTERFACE_MESH_CONNECTIVITY%ELEMENTS_CONNECTIVITY(I,J)%XI)
          ENDIF
        ELSE
          CALL FLAG_ERROR("Interface mesh connectivity element connectivity is being deallocated before allocation.",ERR,ERROR,*999)
        ENDIF      
